...
{
    public class RegroupByUintInclusionMutator : Mutator
    {
        ...
        private MutableField<uint> m_Discriminant = new MutableField<uint>() { AbsoluteKey = "Scores.TeamID" };
        [Controllable(LabelText = "Discriminant")]
        public MutableField<uint> Discriminant { get { return m_Discriminant; } }

        private MutableTarget m_EntriesTarget = new MutableTarget() { AbsoluteKey = "TeamScores" };
        [Controllable(LabelText = "Entries Target")]
        public MutableTarget EntriesTarget { get { return m_EntriesTarget; } }

        private MutableField<string> m_CsvRangeOrKeyword = new MutableField<string>() { LiteralValue = "All" };
        [Controllable(LabelText = "Csv, Range, Or All")]
        public MutableField<string> CsvRangeOrKeyword { get { return m_CsvRangeOrKeyword; } }

        public RegroupByUintInclusionMutator()
        {
        ...
        public static Func<uint, bool> InclusionFunc(string inclusionString)
        {
            uint csvFirst = 0;
            ...
            else if (uint.TryParse(inclusionString, out csvFirst))
            {
                return index => index == csvFirst;
            }
            else if (inclusionString.Contains(','))
            {
                var localCsv = (from str in inclusionString.Split(',') select uint.Parse(str.Trim())).ToList();
            ...
            else if (inclusionString.Contains('.'))
            {
                ...
                var startIndex = uint.Parse(inclusionString.Substring(0, splitPosition).Trim());
                var endIndex = uint.Parse(inclusionString.Substring(splitPosition + 2).Trim());
        ...
        private static bool IndexInRange(uint index, uint min, uint max)
        {
            return index >= min && index <= max;
        }

        private static bool IndexInCsv(uint index, List<uint> csv)
        {
