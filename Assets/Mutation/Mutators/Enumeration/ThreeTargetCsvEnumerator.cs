...
{
    public class ThreeTargetCsvEnumerator : ChainNode
    {
        ...
        public ThreeTargetCsvEnumerator()
        {
        ...
        public override IEnumerator ReceivePayload(VisualPayload payload)
        {
            foreach (var entry in CsvField.GetEntries(payload.Data))
            {
                ...
                NumberOfElementsTarget.SetValue(Mathf.FloorToInt(values.Length / 3f), entry);

                for (int i = 0; i < values.Length / 3; i++)
                {
                    Entry1Target.SetValue(values[3 * i], entry);
                    Entry2Target.SetValue(values[3 * i + 1], entry);
                    Entry3Target.SetValue(values[3 * i + 2], entry);
